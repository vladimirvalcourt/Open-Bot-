...
type SecretVault = Record<string, string>;
let vaultCache: SecretVault | undefined;

function helperExecutable() {
  const candidates = [process.env.OMB_KEYCHAIN_HELPER, resolve("electron/resources/keychain-helper")].filter((value): value is string => Boolean(value));
...
export function secretAccount(section: string, field: string, id?: string) {
  const safe = [section, id, field].filter(Boolean).join(".").replace(/[^A-Za-z0-9._-]/g, "-");
  return safe.slice(0, 220);
}

export function parseSecretVault(value: string): SecretVault {
  try {
    const parsed = JSON.parse(value);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return {};
    return Object.fromEntries(Object.entries(parsed).filter(([account, secret]) =>
      /^[A-Za-z0-9._-]{1,220}$/.test(account) && typeof secret === "string" && secret.length > 0 && secret.length <= 1_000_000,
    )) as SecretVault;
  } catch {
    return {};
  }
...
function helper(operation: "get" | "set" | "delete", account: string, input?: string) {
...
function saveVault(next: SecretVault) {
  const encoded = JSON.stringify(next);
  if (Buffer.byteLength(encoded, "utf8") > 1_000_000) return false;
 ...
 */
export function migrateLegacyKeychainSecrets(accounts: string[]) {
  ...
  if (!migrated.length) return true;
  if (!saveVault(next)) return false;
  for (const account of migrated) helper("delete", account);
  return true;
}

export function readKeychainSecret(account: string) {
  if (!keychainEnabled()) return undefined;
  return vault()[account] || undefined;
}

export function writeKeychainSecret(account: string, value: string) {
  if (!keychainEnabled()) return false;
  if (!value) return deleteKeychainSecret(account);
  const current = vault();
  return saveVault({ ...current, [account]: value });
}

export function deleteKeychainSecret(account: string) {
  if (!keychainEnabled()) return false;
  const current = vault();
  if (!(account in current)) return true;
  const next = { ...current };
  delete next[account];
