...
export function compactTranscript(turns: TranscriptTurn[], maxBytes = 96_000, keepBytes = 48_000): TranscriptTurn[] {
  if (turns.reduce((sum, item) => sum + bytes(item.text), 0) <= maxBytes) return turns;
  let size = 0;
  let cut = turns.length;
  for (let index = turns.length - 1; index >= 0; index--) {
    size += bytes(turns[index].text);
    if (size > keepBytes) break;
    if (turns[index].role === "user") cut = index;
  }
  if (cut <= 0 || cut >= turns.length) cut = Math.max(1, turns.length - 8);
