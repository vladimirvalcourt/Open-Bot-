...
export function configured(cfg: AppConfig) {
...
export function validMachine(value: unknown): value is string {
  if (value === "" || value === undefined) return true;
...
export function runGh(cfg: AppConfig, args: string[], timeoutMs = 120_000): Promise<RunResult> {
...
function requireConfig(cfg: AppConfig) {
  if (!cfg.codespaces?.token) throw new Error("GitHub Codespaces token is not configured");
  if (!validRepository(cfg.codespaces.repository)) throw new Error("Codespaces repository must be owner/repo");
  if (!validBranch(cfg.codespaces.branch)) throw new Error("Codespaces branch contains unsupported characters");
  if (!validMachine(cfg.codespaces.machine)) throw new Error("Codespaces machine name contains unsupported characters");
  return cfg.codespaces.repository.trim();
}

function displayName(botId: string) {
...
async function list(cfg: AppConfig): Promise<CodespaceSummary[]> {
  ...
  if (result.exitCode !== 0) throw new Error(result.stderr.trim() || "Could not list GitHub Codespaces");
  try {
...
export async function find(cfg: AppConfig, botId: string) {
  const wanted = displayName(botId);
  return (await list(cfg)).find((item) => item.displayName === wanted) ?? null;
}

export async function status(cfg: AppConfig, botId: string) {
  if (!configured(cfg)) return { configured: false, provider: "codespaces", computer: null, capabilities: { screenshot: false } };
...
async function waitAvailable(cfg: AppConfig, botId: string, budgetMs = 120_000) {
  const deadline = Date.now() + budgetMs;
  while (Date.now() < deadline) {
    const codespace = await find(cfg, botId);
    if (codespace && /available|running/i.test(codespace.state ?? "")) return codespace;
...
export async function provision(cfg: AppConfig, botId: string, _botName: string) {
  const repository = requireConfig(cfg);
  ...
  if (!codespace) {
    ...
    if (cfg.codespaces?.branch?.trim()) args.push("--branch", cfg.codespaces.branch.trim());
    if (cfg.codespaces?.machine?.trim()) args.push("--machine", cfg.codespaces.machine.trim());
    const created = await runGh(cfg, args, 180_000);
    if (created.exitCode !== 0) throw new Error(created.stderr.trim() || "GitHub Codespace creation failed");
  }
  codespace = await waitAvailable(cfg, botId);
  if (!codespace) throw new Error("GitHub Codespace did not become available within two minutes");
  return {
...
export async function join(cfg: AppConfig, botId: string) {
  const codespace = await find(cfg, botId);
  if (!codespace) throw new Error("no Codespace yet — provision it first");
  const ready = await waitAvailable(cfg, botId);
  if (!ready) throw new Error("the Codespace did not wake in time");
...
export async function sleep(cfg: AppConfig, botId: string) {
  const codespace = await find(cfg, botId);
  if (!codespace) throw new Error("no Codespace for this bot");
  const result = await runGh(cfg, ["codespace", "stop", "--codespace", codespace.name], 60_000);
  if (result.exitCode !== 0) throw new Error(result.stderr.trim() || "Could not stop the Codespace");
  return { ok: true };
}

export async function runRemote(cfg: AppConfig, codespaceName: string, command: string, timeoutMs = 120_000) {
  if (!CODESPACE_NAME.test(codespaceName)) throw new Error("invalid Codespace name");
...
export async function exec(cfg: AppConfig, botId: string, command: string) {
  let codespace = await find(cfg, botId);
  if (!codespace) throw new Error("no Codespace for this bot yet");
  if (!/available|running/i.test(codespace.state ?? "")) codespace = await waitAvailable(cfg, botId, 90_000);
  if (!codespace) throw new Error("Codespace did not wake");
...
export async function integration(cfg: AppConfig, botId: string, proxyPath: string, nodeCommand: string, nodeEnv: Record<string, string>) {
  const codespace = await find(cfg, botId);
