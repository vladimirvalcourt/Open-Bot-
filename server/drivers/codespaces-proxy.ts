...
const result = (id: unknown, text: string, isError = false) => send({
  jsonrpc: "2.0", id, result: { content: [{ type: "text", text }], ...(isError ? { isError: true } : {}) },
});

function run(command: string, timeoutMs = 120_000): Promise<{ code: number | null; stdout: string; stderr: string }> {
  return new Promise((resolveRun, reject) => {
    if (!token || !validName.test(codespace)) return reject(new Error("Codespaces integration is not configured"));
...
async function handle(message: any) {
  ...
  if (method === "initialize") return send({ jsonrpc: "2.0", id, result: { protocolVersion: message.params?.protocolVersion ?? "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "openmausbot-codespaces", version: "1" } } });
  if (String(method ?? "").startsWith("notifications/")) return;
  if (method === "tools/list") return send({ jsonrpc: "2.0", id, result: { tools: TOOLS } });
  if (method === "tools/call") {
    const name = message.params?.name;
    if (!TOOLS.some((tool) => tool.name === name)) return result(id, `Unknown tool: ${name}`, true);
    try {
      const command = name === "computer_state"
        ? "printf 'codespace=%s\\n' \"$CODESPACE_NAME\"; uname -a; pwd"
        : String(message.params?.arguments?.command ?? "");
      if (!command) return result(id, "computer_exec requires a command", true);
