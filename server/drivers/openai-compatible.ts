// Generic OpenAI-compatible Chat Completions driver. Each configured instance
// owns one endpoint, one write-only key, and one explicit model. This covers
// DeepSeek, Moonshot/Kimi API, Z.AI/GLM, MiniMax, Qwen/DashScope, SiliconFlow,
// OpenRouter, and other compatible gateways without vendor-specific code.
import type {
  DriverCreateInput,
  ProviderDriver,
  ProviderInstance,
...
function decodeConfig(raw: unknown): OpenAiCompatibleConfig {
  ...
  if (!baseUrl || !model) throw new Error("API provider needs a base URL and model");
...
export const OpenAiCompatibleDriver: ProviderDriver<OpenAiCompatibleConfig> = {
  ...
  async create(input: DriverCreateInput<OpenAiCompatibleConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      void (async () => {
        try {
          ...
          if (result.text.trim()) emit({ ...base(turn.threadId, turnId), type: "item.completed", itemType: "assistant_text", text: result.text });
    ...
    return {
      ...
      snapshot: async (): Promise<ProviderSnapshot> => apiKey
