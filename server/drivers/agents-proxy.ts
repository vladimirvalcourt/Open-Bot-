...
import { resolveNamedItem } from "../capabilities.ts";
...
const WORKSPACE_TOOLS = [
  {
    ...
    description:
      "Return the capabilities currently available to this bot and the recommended fallback order. Use when you are unsure whether connected apps, browser, computer, bots, memory, or routines are available.",
  ...
  },
  {
    name: "search_memory",
    description:
      "Search user-managed workspace memory for preferences, facts, procedures, and project context before asking the user for information they may already have saved.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "remember",
    description:
      "Save something to user-managed memory only when the user explicitly asked to remember or save it. This creates persistent state; never infer consent from ordinary conversation.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "create_routine",
    description:
      "Create persistent recurring work for this bot after the user explicitly requests scheduling. Resolve or ask for any unclear cadence, timezone, or wall-clock time before calling.",
    inputSchema: {
      type: "object",
      properties: {
...
async function callTool(name: string, args: Json): Promise<{ text: string; isError?: boolean }> {
  ...
  if (name === "ask_bot") {
    ...
    if (!message || (!toBotId && !botName)) return { text: "ask_bot needs message and either bot_id or bot_name.", isError: true };
    if (!toBotId) {
      const roster = await api(`/api/internal/agents?self=${encodeURIComponent(BOT_ID)}`);
      const bots = ((roster.bots as Array<Json>) ?? []).map((bot) => ({ id: String(bot.id), name: String(bot.name) }));
  ...
  if (name === "search_memory") {
    ...
    if (!query) return { text: "search_memory needs a query.", isError: true };
    ...
    const items = (r.memories as Array<Json>) ?? [];
    if (!items.length) return { text: "No matching saved memory." };
    return { text: items.map((item) => `- [${item.kind}] ${item.text}`).join("\n") };
  }
  if (name === "remember") {
    const text = String(args.text ?? "").trim();
    if (!text) return { text: "remember needs text.", isError: true };
    const r = await api(`/api/internal/memory`, {
      method: "POST",
      body: JSON.stringify({ botId: BOT_ID, text, scope: args.scope, kind: args.kind }),
    });
    const item = r.memory as Json;
  ...
  if (name === "list_routines") {
    ...
    const items = (r.routines as Array<Json>) ?? [];
    if (!items.length) return { text: "This bot has no scheduled routines." };
