...
const fail = (id: unknown, message: string) => ok(id, { content: [{ type: "text", text: message }], isError: true });

async function call(action: string, input: Json) {
  ...
  if (!response.ok) throw new Error(result.error ?? `browser bridge HTTP ${response.status}`);
...
async function handle(message: Json) {
  ...
  if (method === "initialize") return ok(id, { protocolVersion: params.protocolVersion ?? "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "openmausbot-browser", version: "1" } });
  if (method === "notifications/initialized" || method === "notifications/cancelled") return;
  if (method === "ping") return ok(id, {});
  if (method === "tools/list") return ok(id, { tools: TOOLS });
  if (method === "tools/call") {
    if (!TOOLS.some((tool) => tool.name === params.name)) return fail(id, `Unknown browser tool: ${params.name}`);
    try { return ok(id, await call(params.name, params.arguments ?? {})); }
    catch (error) { return fail(id, error instanceof Error ? error.message : String(error)); }
  }
