...
export interface BackupEnvelope {
  format: "openmausbot-encrypted-backup";
  version: 1;
  createdAt: number;
...
function key(passphrase: string, salt: Buffer) {
  if (passphrase.length < 10) throw Object.assign(new Error("backup passphrase must be at least 10 characters"), { status: 400 });
  return scryptSync(passphrase, salt, 32, { N: 1 << 15, r: 8, p: 1, maxmem: 64 * 1024 * 1024 });
}

export function createEncryptedBackup(passphrase: string): BackupEnvelope {
  ...
  for (const absolute of filesUnder(DATA_DIR)) {
    ...
    if (!SAFE_FILE.test(path)) continue;
    const data = readFileSync(absolute);
    bytes += data.length;
    if (bytes > MAX_BACKUP_BYTES) throw Object.assign(new Error("backup exceeds the 120 MB local limit"), { status: 413 });
...
export function restoreEncryptedBackup(envelope: BackupEnvelope, passphrase: string) {
  if (envelope?.format !== "openmausbot-encrypted-backup" || envelope.version !== 1) throw Object.assign(new Error("unsupported backup format"), { status: 400 });
  let payload: any;
  try {
    const salt = Buffer.from(envelope.salt, "base64"); const iv = Buffer.from(envelope.iv, "base64");
  ...
  if (!Array.isArray(payload?.files) || payload.files.length > 10_000) throw Object.assign(new Error("backup file list is invalid"), { status: 400 });
  let bytes = 0; let restored = 0;
  for (const item of payload.files) {
    const path = String(item?.path ?? "");
    if (!SAFE_FILE.test(path)) throw Object.assign(new Error("backup contains an unsafe path"), { status: 400 });
    const data = Buffer.from(String(item?.data ?? ""), "base64");
    bytes += data.length;
    if (bytes > MAX_BACKUP_BYTES) throw Object.assign(new Error("backup expands beyond the 120 MB local limit"), { status: 413 });
    const destination = resolve(DATA_DIR, path);
    if (!destination.startsWith(resolve(DATA_DIR) + sep)) throw Object.assign(new Error("backup path escaped the data directory"), { status: 400 });
    mkdirSync(dirname(destination), { recursive: true, mode: 0o700 });
    const temp = `${destination}.restore-${process.pid}`;
    writeFileSync(temp, data, { mode: 0o600 }); renameSync(temp, destination); try { chmodSync(destination, 0o600); } catch {}
    restored++;
  }
