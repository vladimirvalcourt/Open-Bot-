...
export class AttachmentStore {
    items = [];
    constructor() { mkdirSync(DIR, { recursive: true, mode: 0o700 }); try {
        const value = JSON.parse(readFileSync(META, "utf8"));
        this.items = Array.isArray(value) ? value : [];
    }
    catch { } }
    saveIndex() { const tmp = `${META}.tmp`; writeFileSync(tmp, JSON.stringify(this.items, null, 2), { mode: 0o600 }); renameSync(tmp, META); try {
        chmodSync(META, 0o600);
    }
    catch { } }
    save(input) {
        const name = basename(input.name).replace(/[^a-zA-Z0-9._ -]/g, "_").slice(0, 180);
        if (!name || !ALLOWED.has(extname(name).toLowerCase()))
            throw Object.assign(new Error("unsupported attachment type"), { status: 400 });
        const data = Buffer.from(input.base64, "base64");
        if (!data.length || data.length > 20 * 1024 * 1024)
            throw Object.assign(new Error("attachment must be between 1 byte and 20 MB"), { status: 413 });
        const id = newId();
        ...
        const item = { id, name, mime: String(input.mime || "application/octet-stream").slice(0, 120), size: data.length, path, createdAt: Date.now() };
    ...
    resolve(ids) {
        ...
        return unique.map((id) => this.items.find((item) => item.id === id)).filter((item) => {
            if (!item)
                return false;
    ...
    public(item) { const { path: _path, ...safe } = item; return safe; }
}
