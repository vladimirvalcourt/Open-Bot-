...
import { Composio } from "./composio-runtime.js";
 ...
 */
export function connectedAppsInstructions() {
    return [
...
function apiKey(cfg) {
    ...
    if (!value)
        throw new Error("no Composio Platform API key configured");
    return value;
}
function localUserId(cfg) {
    if (cfg.composio?.userId)
        return cfg.composio.userId;
    const userId = `openmausbot_${randomUUID()}`;
    cfg.composio = { ...cfg.composio, userId };
    saveConfig({ composio: { userId } });
    return userId;
}
export async function platformSession(cfg) {
    const key = apiKey(cfg);
    if (cached?.apiKey === key)
        return cached.session;
    const client = new Composio({ apiKey: key });
    let session = null;
    if (cfg.composio?.sessionId) {
        try {
            session = await client.use(cfg.composio.sessionId, { mcp: true });
        }
        catch { }
    }
    if (!session) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (Array.isArray(items) && items.length) {
                    const cards = items.map((t) => ({ slug: (t.slug ?? t.key ?? t.name ?? "").toLowerCase(), label: t.name ?? t.slug ?? "", blurb: (t.meta?.description ?? t.description ?? "").slice(0, 90), logo: t.meta?.logo ?? t.logo ?? null, domain: null }));
